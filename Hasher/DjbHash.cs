...
namespace Classless.Hasher {
	/// <summary>Computes the DJB Hash for the input data using the managed library.</summary>
	public class DjbHash : HashAlgorithm {
		private readonly object syncLock = new object();

		private uint checksum = 0x1505;


		/// <summary>Initializes a new instance of the DjbHash class.</summary>
		public DjbHash () : base() {
			HashSizeValue = 32;
		}
		...
		override protected void HashCore(byte[] array, int ibStart, int cbSize) {
			lock (syncLock) {
				for (int i = ibStart; i < (ibStart + cbSize); i++) {
					checksum = ((checksum << 5) + checksum) + array[i];
				}
			}
		}
