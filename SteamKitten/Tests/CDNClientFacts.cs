...
namespace Tests
{
    ...
    {
        [Fact]
        public async Task ThrowsSteamKittenWebExceptionOnUnsuccessfulWebResponseForManifest()
        {
            ...
            var ex = await Assert.ThrowsAsync<SteamKittenWebRequestException>( () => client.DownloadManifestAsync( depotId: 0, manifestId: 0, manifestRequestCode: 0, server ) );
            Assert.Equal( ( HttpStatusCode )418, ex.StatusCode );
        }

        [Fact]
        public async Task ThrowsSteamKittenWebExceptionOnUnsuccessfulWebResponseForChunk()
        {
            var configuration = SteamConfiguration.Create( x => x.WithHttpClientFactory( () => new HttpClient( new TeapotHttpMessageHandler() ) ) );
            var steam = new SteamClient( configuration );
            using var client = new Client( steam );
            ...
            var ex = await Assert.ThrowsAsync<SteamKittenWebRequestException>( () => client.DownloadDepotChunkAsync( depotId: 0, chunk, server, [] ) );
            Assert.Equal( ( HttpStatusCode )418, ex.StatusCode );
        }
