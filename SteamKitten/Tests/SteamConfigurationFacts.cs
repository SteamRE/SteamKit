...
namespace Tests
{
    public class SteamConfigurationDefaultFacts
    {
        ...
        public void DefaultHttpClientFactory()
        {
            ...
            var SteamKittenAssemblyVersion = typeof( SteamClient ).Assembly.GetName().Version;
            Assert.Equal( "SteamKitten/" + SteamKittenAssemblyVersion.ToString( fieldCount: 3 ), client.DefaultRequestHeaders.UserAgent.ToString() );
        }
    ...
    public class SteamConfigurationConfiguredObjectFacts
    {
        public SteamConfigurationConfiguredObjectFacts()
        {
            configuration = SteamConfiguration.Create(b =>
                b.WithDirectoryFetch(false)
                 ...
                 .WithHttpClientFactory(() => { var c = new HttpClient(); c.DefaultRequestHeaders.Add("X-SteamKitten-Tests", "true"); return c; })
        ...
        {
            using var client = configuration.HttpClientFactory();
            Assert.Equal( "true", client.DefaultRequestHeaders.GetValues( "X-SteamKitten-Tests" ).FirstOrDefault() );
        }
