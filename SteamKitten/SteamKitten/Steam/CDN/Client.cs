...
namespace SteamKitten.CDN
{
    ...
    public sealed partial class Client : IDisposable
    {
        ...
        /// <exception cref="SteamKittenWebRequestException">A network error occurred when performing the request.</exception>
        public async Task<DepotManifest> DownloadManifestAsync( uint depotId, ulong manifestId, ulong manifestRequestCode, Server server, byte[]? depotKey = null, Server? proxyServer = null, string? cdnAuthToken = null )
        {
            ...
            try
            {
                ...
                {
                    throw new SteamKittenWebRequestException( $"Response status code does not indicate success: {response.StatusCode:D} ({response.ReasonPhrase}).", response );
                }
        ...
        public async Task<int> DownloadDepotChunkAsync( uint depotId, DepotManifest.ChunkData chunk, Server server, byte[] destination, byte[]? depotKey = null, Server? proxyServer = null, string? cdnAuthToken = null )
        {
            ...
            try
            {
                ...
                {
                    throw new SteamKittenWebRequestException( "Response does not have Content-Length and chunk.CompressedLength is not set.", response );
                }
