...
namespace SteamLanguageParser
{
    class CSharpGen : ICodeGen
    {
        ...
        public void EmitSerialBase(StringBuilder sb, int level)
        {
            ...
            sb.AppendLine(padding + "\tMemoryStream serialize();");
        ...
        private void EmitClassSerializer(ClassNode cnode, StringBuilder sb, int level, int baseSize)
        {
            ...
            sb.AppendLine();
            sb.AppendLine(padding + "public MemoryStream serialize()");
            sb.AppendLine(padding + "{");
            ...
            if (cnode.Parent != null)
            {
                sb.AppendLine(padding + "\tMemoryStream msHeader = Header.serialize();");
            ...
            foreach (PropNode prop in cnode.childNodes)
            {
                ...
                if (size == 0)
                {
                    if (prop.Flags != null && prop.Flags == "proto")
                    {
                        ...
                        {
                            // early exit
                            sb.AppendLine(padding + "\treturn ms" + GetUpperName(prop.Name) + ";");
                            sb.AppendLine(padding + "}");
                            return;
                        }
                    }
            ...
            sb.AppendLine(padding + "\tMemoryStream msBuffer = new MemoryStream( " + String.Join(" + ", varLengthProps) + " );");
            sb.AppendLine();
            ...
            foreach (PropNode prop in cnode.childNodes)
            {
                ...
                sb.AppendLine(padding + "\twriter.Write( " + typecast + propName + " );");
            }
            ...
            sb.AppendLine(padding + "\treturn msBuffer;");
            sb.AppendLine(padding + "}");
        }
        ...
        private void EmitClassDeserializer(ClassNode cnode, StringBuilder sb, int level, int baseSize)
        {
            ...
            if (baseSize > 0)
            {
                sb.AppendLine(padding + "\tBinaryReader reader = new BinaryReader( ms );");
                sb.AppendLine();
            }
            ...
            foreach (PropNode prop in cnode.childNodes)
            {
                ...
                if (size == 0)
                {
                    if (prop.Flags != null && prop.Flags == "proto")
                    {
                        if (prop.FlagsOpt != null)
                        {
                            sb.AppendLine(padding + "\tusing( MemoryStream ms" + GetUpperName(prop.Name) + " = new MemoryStream( reader.ReadBytes( " + GetUpperName(prop.FlagsOpt) + " ) ) )");
                ...
                else
                {
                    ...
                    string call = "reader.Read" + readerTypeMap[typestr] + "()";

                    if (!String.IsNullOrEmpty(prop.FlagsOpt))
                    {
                        call = "reader.Read" + readerTypeMap[typestr] + "s( " + prop.FlagsOpt + " )";
                    }
